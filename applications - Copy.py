...
def hybrid_search(function,funcprime, (args),dx,dy,x0,y0,N,max_iterations,xmin,xmax,ymin,ymax,tolerance): 
	...
	for i in range (max_iterations):
		...
		if (psign1 > 0 and x_naught==xmin) or (psign1 < 0 and x_naught==xmax) or (psign1==0): #x0 is found
			xpts = x_naught
		if (psign2 > 0 and y_naught==ymin) or (psign2 < 0 and y_naught==ymax) or (psign2==0): #y0 is found
			ypts = y_naught
		#set xpts
		if psign1 < 0 and xpts==None:
			xpts = np.linspace(x_naught,min(x_naught+dex_x,xmax),N)
		elif psign1 > 0 and xpts==None:
			xpts = np.linspace(max(x_naught-dex_x,xmin),x_naught,N)
		#elif psign1==0:
		#	xpts = np.linspace(max(x_naught-dex_x,xmin),min(x_naught+dex_x,xmax),N)
		#set ypts
		if psign2 < 0 and ypts==None:
			ypts = np.linspace(y_naught,min(y_naught+dex_y,ymin),N)
		elif psign2 > 0 and ypts==None:
			ypts = np.linspace(max(y_naught-dex_y,ymin),y_naught,N)
		#elif psign2==0:

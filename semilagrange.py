...
#INPUTS
dx = 0.05#1/300 #these taken from Gueant's paper
dt = dx
xmin = 0
xmax = 1
T = 1
...
tolerance = 1e-6
epsilon = 2*dt #for use in convolution thing
molly = 1
quad_order = 15
...
def mollifier(x_val): #Evans' mollifier
	if abs(x_val) < 1:
		return 0.443994*np.exp(1/(x_val**2 - 1))
	else:
		return 0
...
for n in range (0,Niter):
	...
	for k in range (K-2,-1,-1):
		...
		for i in range (0,I):
			...
			#tmp = min_approx1(tau,(i,v_tmp,x)) #this dominates time consumption! and works
	...
	if (vlinfty[n] < tolerance):
		print "Method converged with final change" , vlinfty[n]
		kMax = n
		break
	...
	for k in range(0,K-1):
		...
		if molly==1:
			v_grad = mollify_array(np.gradient(v[(I*k):(I*k+I)],dx),epsilon,x,gll_x,gll_w) #this isn't completely fine
			#m[(I*(k)):(I*(k)+I)] = mollify_array(m[(I*(k)):(I*(k)+I)],epsilon,x,gll_x,gll_w)
		else:

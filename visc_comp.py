...
dexes = [1/10, 1/20, 1/30, 1/40, 1/50, 1/60]
DT = .1
DEX_LEN = len(dexes)
epses = [None]*DEX_LEN
solutions = [None]*DEX_LEN
TEST_NAME = "mfg1dScaled"
...
for i in range(DEX_LEN-1):
	e1[i] = np.linalg.norm(finest-interpolates[i],ord=1)
	e2[i] = np.linalg.norm(finest-interpolates[i],ord=2)
	einf[i] = np.linalg.norm(finest-interpolates[i],ord=np.inf)

...
def HJB_diffusion_implicit(time,x,y,a1,a2,dx,dy,dt,south,north,west,east,nulled): #this works, at least for constant diffusion and without D12; CORNERS ARE BAD FOR D12
	I,J = x.size,y.size
	...
	F_here = 1+dt/dx2*D11 + dt/dy2*D22- dt/dxy*abs(D12) #[i,i]
	...
	for i in range(0,I*J):
		...
		d22 = D22[i]
		d12 = D12[i]
		if ismember(i,north): #allows (i,j+1)
			if not ismember(i,west):
				F_south_west[i] += dt/dxy*min(d12,0)/2
			if not ismember(i,east):
				F_south_east[i] = -dt/dxy*max(d12,0)/2
		if ismember(i,west): #allows (i-1,j) 
			F_east[i] += -dt*(d11/dx2-abs(d12)/dxy)/2
			if not ismember(i,north):
				F_north_east[i] += dt/dxy*min(d12,0)/2
			if not ismember(i,south):
				F_south_east[i] += -dt/dxy*max(d12,0)/2
		if ismember(i,east): #allows (i+1,j)
			F_west[i] += -dt*(d11/dx2-abs(d12)/dxy)/2
			if not ismember(i,north):
				F_north_west[i] += -dt/dxy*max(d12,0)/2
			if not ismember(i,south):
				F_south_west[i] += dt/dxy*min(d12,0)/2
		if ismember(i,south) and ismember(i,west): #allows (i-1,j-1)
			F_north_east[i] += -dt/dxy*max(d12,0)/2
		if ismember(i,north) and ismember(i,east): #allows (i+1,j+1)
			F_south_west[i] += -dt/dxy*max(d12,0)/2
		if ismember(i,south) and ismember(i,east): #allows (i+1,j-1)
			F_south_east[i] += dt/dxy*min(d12,0)/2
		if ismember(i,west) and ismember(i,north): #allows (i-1,j+1)
			F_north_west[i] += dt/dxy*min(d12,0)/2
...
def HJB_convection_explicit(time,x,y,a1,a2,dx,dy,dt,south,north,west,east,nulled): #this should work, but also needs BC
	...
	F_here = 1-dt*(abs(f1_array)/dx+abs(f2_array)/dy) #[i,i]
	F_north = np.zeros(I*J) #[i,i+I]
	F_south = np.zeros(I*J) #[i,i-I]
	F_west = np.zeros(I*J) #[i,i-1]
	F_east = np.zeros(I*J) #[i,i+1]
...
def FP_convection_explicit_classic(time,x,y,a1,a2,dx,dt):
	...
	F_here = 1-dt/dx*(abs(F1)+abs(F2)) #[i,i]
	F_north = np.zeros(I*J) #[i,i+I]
	F_south = np.zeros(I*J) #[i,i-I]
	F_west = np.zeros(I*J) #[i,i-1]
	F_east = np.zeros(I*J) #[i,i+1]
	...
	F_south[not_south] = -dt/dx*F2min[not_south-I]
	F_north[not_north] = dt/dx*F2max[not_north+I]
	F_east[not_east] = dt/dx*F1max[not_east+1]
	F_west[not_west] = -dt/dx*F1min[not_west-1]
	#for i in range(0,I*J):
...
def FP_convection_explicit_interpol(time,x,y,a1,a2,dx,dt,south,north,west,east,nulled):
	...
	Fl_here = 1-dt/dx*(-np.minimum(F_south,zero)-np.minimum(F_west,zero)+np.maximum(F_north,zero)+np.maximum(F_east,zero))

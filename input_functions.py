...
#	return 2.5*x_array #Ornstein FP test
	...
	#print a_array
	#print pos_ind
	if pos_ind[0].size is not 0:
		if len(pos_ind)==2:
			output[pos_ind] = a_array[pos_ind]*(1-0.5*np.sin(np.pi*time)-0.25*x_array[pos_ind[1]])
		else:
			output[pos_ind] = a_array[pos_ind]*(1-0.5*np.sin(np.pi*time)-0.25*x_array[pos_ind])
	if neg_ind[0].size is not 0:
...
def Sigma_global(time,x_array,a_array): #any of these will do for the HJB test
	...
	one = np.ones(a_array.shape)
	return .1*one
	#return 0.1*abs(a_array)#+0.1 #mfg1d
	if isinstance(a_array, np.float64):
		#return max(a_array,0)*np.exp(-time) + min(a_array,0)*0.1
		if a_array > 0:
			return 0.1*(1+max(a_array,0)*(1-x_array))
		else:
			return 0
	#one = np.ones(a_array.shape)
	output = np.zeros(a_array.shape)
	...
	if pos_ind[0].size is not 0:
		if len(pos_ind)==2:
			#output[pos_ind] = a_array[pos_ind]*(1-0.5*np.sin(np.pi*time)-0.25*x_array[pos_ind[1]])
			#output[pos_ind] = 2*0.05*(1+a_array[pos_ind]*(1-x_array[pos_ind[1]]))
			output[pos_ind] = 0.1*(1+a_array[pos_ind]*(1-x_array[pos_ind[1]]))
		else:
			#output[pos_ind] = a_array[pos_ind]*(1-0.5*np.sin(np.pi*time)-0.25*x_array[pos_ind])
			output[pos_ind] = 0.1*(1+a_array[pos_ind]*(1-x_array[pos_ind]))
	#if neg_ind[0].size is not 0:
	#	output[neg_ind] = .0*one[neg_ind]
	#print output
	#print ss
	return output
#	return 0.05*(1+4*(1-x_array)*abs(a_array))#+0.1 #mfg1d
...
def Hamiltonian_Derivative_vectorised(alphas,time,x,u_up,u_down,m,dx): 
	...
	#L = L_global(time,x,alphas,m,dx)
	zero = np.zeros(alphas.shape)
	#return alphas + u_down*np.maximum(np.sign(alphas),zero) + u_up*np.minimum(np.sign(alphas),zero) #isolation game, cmfg
	return alphas/(1+0.1*m) + np.exp(-time)*u_down*np.maximum(np.sign(alphas),zero) + .1*u_up*np.minimum(np.sign(alphas),zero) + .01*alphas*(u_up-u_down)/dx  #isolation game, mfg

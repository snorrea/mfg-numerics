...
if LOAD_WRITE: #load best solution with parameters
	...
	for file in glob.glob("*.txt"):
		...
		pop_DT = float(pop[2])
		pop_eps = float(pop[3])
		if pop_dx==dx and pop_DT==DT:
			if pop_eps < best_eps:
				best_dx = pop_dx
				best_DT = pop_DT
				best_eps = pop_eps
	if best_eps is not 42:
		...
		m = np.loadtxt("./test_" + dx_string + "_" + DT_string + "_" + eps_string + "_" + ".txt")
...
epses = 40
epsel = start_eps*np.linspace(1,0,epses)**2
...
for NN in range(epses):
	...
	#compute norms of stuff
		...
		if (ml1[n+kMax] < tolerance):
			print "Method converged with final change" , mlinfty[n+kMax]
...
fig7 = plt.figure(7)
plt.plot(x,m[0:I],label="Initial distribution")

...
epses = 21
end_eps = np.linspace(.1,1.,epses)
start_eps = np.linspace(.7,1.,epses)
eps_exp = 1
...
if LOAD_WRITE: #load best solution with parameters
	...
	for file in glob.glob("*.txt"):
		...
		pop_dx = float(pop[1])
		pop_DT = float(pop[2])
		pop_seps = float(pop[3])
		if abs(pop_dx-dx)<1e-8 and pop_DT==DT and pop[0]=="decr":
			pop_eeps = float(pop[4])
			if pop_eeps > best_eeps:
				best_dx = pop_dx
				best_DT = pop_DT
				best_eeps = pop_eeps
				best_seps = pop_seps
	if best_seps is not -1 and start_eps[0] < best_seps:
		start_eps = best_eps+deps
		BETTER = True
		dx_string = "%.8f" % best_dx
		DT_string = "%.8f" % best_DT
		eeps_string = "%.8f" % best_seps
		seps_string = "%.8f" % best_eeps
		print "Loading previous computation result..."
		m = np.loadtxt("./decr_" + dx_string + "_" + DT_string + "_" + seps_string + "_" + eeps_string + "_" + ".txt")
...
for NN in range(epses):
	...
	damp_thing = start_eps[NN] + (end_eps[NN]-start_eps[NN])*t**eps_exp
	#print damp_thing
	#print ss
		...
		for k in range (K-1,0,-1):  #this is how it has to be...
			...
			if NICE_DIFFUSION==0:
				...
				#	LHS_HJB = mg.hjb_diffusion(k*dt,x,a_tmp,dt,dx)
					LHS_HJB = mg.hjb_diffusion_av(k*dt,x,a_tmp,dt,dx,(1-start_eps[NN])*.1)
		...
		for k in range(0,K-1): #COMPUTE M WHERE WE DO NOT ALLOW AGENTS TO LEAVE SUCH THAT m(-1) = m(N+1) = 1 ALWAYS
			...
			if NICE_DIFFUSION==0:
				...
				if n==0 and k==0:
					...
					LHS_FP = mg.fp_fv_diffusion_av(k*dt,x,a_tmp,dt,dx,(1-start_eps[NN])*.1)
...
if LOAD_WRITE: #load best solution with parameters
	...
	if Success: #write NN to file
		...
		dt_string = "%.8f" % DT
		eps_string = "0_0"
		filename = "decr_" + dx_string + "_" + dt_string + "_" + eps_string + "_" + ".txt"
		...
		dx_string = "%.8f" % dx
		dt_string = "%.8f" % DT
		eps_string = "%.8f_%.8f" % start_eps[NN-1] %end_eps[NN-1]
		filename = "decr_" + dx_string + "_" + dt_string + "_" + eps_string + "_" + ".txt"
		np.savetxt(filename, m_best, fmt='%.18e', delimiter=' ', newline='\n', header='', footer='', comments='# ')
...
for NNs in range(NN+1):
	string_thing = "Solution with start:", "%.4f" %start_eps[NNs] + "and end:", "%.4f" %end_eps[NNs]

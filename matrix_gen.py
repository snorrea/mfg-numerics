...
#####################
def m_matrix_implicit(f1_array,f2_array,D11,D22,D12,I,J,dx,dt): #THIS JUST WORKS
	output = np.zeros((I*J,I*J))
	lamb = dt/(dx**2)
	xbound1 = range(0,I)
	ybound1 = range(0,I*J,I)
	...
	for i in range(0,I*J):
		...
		if not ismember_sorted(i,xbound1) and not ismember_sorted(i,xbound2) and not ismember_sorted(i,ybound1) and not ismember_sorted(i,ybound2):
			#center terms
			output[i,i] += dx*(max(0.5*(f1_array[i]+f1_array[i+1]),0)+max(0.5*(f2_array[i]+f2_array[i+1]),0)-min(0.5*(f1_array[i]+f1_array[i-1]),0)-min(0.5*(f2_array[i]+f2_array[i-1]),0)) #term i)
			...
			output[i,i] += 0
			output[i,i] += 0
			output[i,i] += 0.5*(-(D11[i+1]-D11[i]) -0.25*(D11[i]+D11[i+1]+D11[i-I]+D11[i+1-I]) + 0.5*(D12[i]+D12[i+1]+D12[i-I]+D12[i+1-I]) -(D12[i]+D12[i+1]) - (D12[i]+D12[i-I]) + (D22[i]+D22[i-I]) + 0.5*(D22[i]+D22[i+1]+D22[i-I]+D22[i+1-I]) ) #term iii)
			output[i,i+1] += 0.5 #term i)
			output[i,i+1] += 0.5 #term ii)
			output[i,i+1] += 0.5 #term iii)
			output[i,i+I] += 0.5
			output[i,i+I] += 0.5
			output[i,i+I] += 0.5
			output[i,i+1+I] += 0.5
			...
			output[i,i+I] += 0
			output[i,i-1+I] += 0
		elif ismember_sorted(i,ybound1) and ismember_sorted(i,xbound2): #NORTH-WEST
			output[i,i] += 0
			output[i,i+1] += 0
			output[i,i-I] += 0
			output[i,i+1-I] += 0
			...
			output[i,i+I] += 0
			output[i,i+1+I] += 0
		elif ismember_sorted(i,ybound2): #SOUTH
			output[i,i-1] += 0
